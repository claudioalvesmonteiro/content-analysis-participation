...
```{r}
# baixar dados
url <- "https://github.com/claudioalvesmonteiro/publicgov_environment/blob/master/apa_costa_dos_corais/data/atas_presenca_apacc.xlsx?raw=true"
...
presen_insti$Group.1  <- factor(presen_insti$Group.1, levels = unique(presen_insti$Group.1[order(presen_insti$x)]), ordered = TRUE)
```
...
ggplot(
  ...
  labs(x = "", y = "Número de Presentes", title = "Presença das Instituições no Conselho") +
  #geom_label(label = presen_insti$x, size = 2.5, color = "black", fontface = "plain") +
  #theme_light()+
  coord_flip()
...
```{r}
# importar base de dados das categorias
