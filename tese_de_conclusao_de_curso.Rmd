...
library(RQDA); library(readxl); library(readr); library(stringi); library(stringr);
...
dataNetworkProcess <- function(matrix){
  data = data.frame(as.table(matrix))    # tranformar em banco de dados
  # criar base com nomes e numeros dos codigos
  code_names = data.frame(code_full = data$Var1[!duplicated(data$Var1)], code_num = "", code_nome = "")
  
  #
  data = data[!is.na(data$Freq),]        # remover NAs
  data = data[data$Freq != 0,]           # remover casos sem relacao
  # 
...
cod_seq <- regmatches(dataAtua$Var1, gregexpr("[[:digit:]]+", dataAtua$Var1))
dataAtua$nod_cod <- as.numeric(unlist(cod_seq))

# transformar variaveis
dataAtua$Var2 <- as.numeric(as.character(dataAtua$Var2))
dataAtua$Var1 <- as.character(dataAtua$Var1)

# definir nomes dos nodes
nomes2 <- data.frame(nome_nod = code_atua)
nomes2$nome_nod <- as.character(nomes2$nome_nod)

flow_unique2 <- dataAtua[!duplicated(dataAtua$Var1),]

# limpar base
dataAtua <- mutate(dataAtua, clean = ifelse(Var2 == nod_cod | is.na(Freq), 1, 0))
dataAtua <- dataAtua[dataAtua$clean == 0,]
...
flow_unique2$nome_nod <- best_match(as.character(flow_unique2$Var1), nomes2$nome_nod)
...
# criar IDs
dataAtua$IDsource = match(dataAtua$nod_cod, nomes2$nod_cod)-1 
dataAtua$IDtarget = match(dataAtua$Var2, nomes2$nod_cod)-1

# selecionar relacoes entre representantes e atuacao ##
dataAtua <- mutate(dataAtua, IN = NA)
dataAtua$IN[str_detect(dataAtua$Var1, "atua_")] <- "ATUA" 
dataAtua$IN[is.na(dataAtua$IN)] <- "REP" 

dataAtua <- mutate(dataAtua, OUT = "")
paste2_atua <- nomes2$nod_cod[nomes2$grupos == "Atuação nos Debates"]
paste2_rep <- nomes2$nod_cod[nomes2$grupos == "Representante"]
dataAtua$OUT[str_detect(dataAtua$Var2, paste(paste2_atua, collapse = '|'))] <- "ATUA"
dataAtua$OUT[str_detect(dataAtua$Var2, paste(paste2_rep, collapse = '|'))] <- "REP"

dataAtua <- mutate(dataAtua, select = ifelse(IN == OUT, 1, 0))
dataAtua <- dataAtua[dataAtua$select == 0,]


# 
mani1 <- as.factor(dataAtua$Var1)
...
#===== NETWORK3 ======#
network_rep_atua <- 
  forceNetwork(dataAtua, Nodes = nomes2, Source = "IDsource",  Target = "IDtarget",
